...
__init__=function(config,obj,prefix){for(var key in obj){var checked=false;var val=obj[key];if(isset(val.checked)){config.selector.find("input[name='"+key+"']").each(function(){$(this).attr("checked",val.checked);});delete val.checked;}
if(!empty(val)){__init__(config,val,empty(prefix)?key:prefix+"."+key);}
config.selector.find("input[name='"+key+"']").each(function(){$(this).bind("click",{prefix:prefix,name:key,descendants:val},function(e){var checked=$(this).attr("checked");traverse_descendants(config,e.data.descendants,checked);traverse_ancestors(config,e.data.prefix);config.change(e.data.name);});});}
...
config.selector.find("input[name='"+key+"']").each(function(){$(this).attr("checked",checked);});if(config.tristate){config.selector.find("input[name='"+key+"']").each(function(){$(this).attr("indeterminate",false);});}}}
...
break;}
var count=0,checked=0,indeterminate=0;for(var name in obj){config.selector.find("input[name='"+name+"']").each(function(){if($(this).attr("checked")){++checked;}
if($(this).attr("indeterminate")){++indeterminate;}
++count;});}
var name=arr.pop();config.selector.find("input[name='"+name+"']").each(function(){$(this).attr("checked",checked>0);});if(config.tristate){config.selector.find("input[name='"+name+"']").each(function(){$(this).attr("indeterminate",(checked>0&&count!=checked)||(indeterminate>0));});}
